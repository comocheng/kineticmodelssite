    ...
    def print_model_xml(self):
        ...
        for n in range(len(specieslist)):
            ...
            reactiondict["child4_{0}_kin".format(n)] = etree.SubElement(
                reactiondict["child4_{0}".format(n)], "reactionRateLink"
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Expor Django database to prime format")
    parser.add_argument(
        "output", metavar="output", nargs=1, help="location of where to save things"
    )
