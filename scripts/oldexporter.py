...
class xmlReaction:
    def print_reaction_xml(self):
        ...
        sPrimeIDs = [
            ...
            "s00000003",
            "s00000004",
        ]  # each element corresponds to specieslist element
...
class xmlModel:
    def print_model_xml(self):
        ...
        for n in range(len(specieslist)):
            ...
            speciesdict["child3_{0}_thermo".format(n)] = etree.SubElement(
                speciesdict["child3_{0}".format(n)], "thermodynamicDataLink"
            )
            speciesdict["child3_{0}_thermo".format(n)].attrib[
                "preferredKey"
            ] = None  # TODO thermo preferred key (i.e. "L 7/88") for corresponding thermo
            speciesdict["child3_{0}_thermo".format(n)].attrib["primeID"] = thpPrimeID
            if False:  # TODO if transport of specieslist[n] is not None:
                speciesdict["child3_{0}_trans".format(n)] = etree.SubElement(
                    speciesdict["child3_{0}".format(n)], "transportDataLink"
                )
        ...
        for n in range(len(specieslist)):
            ...
            reactiondict["child4_{0}_kin".format(n)] = etree.SubElement(
                speciesdict["child4_{0}".format(n)], "reactionRateLink"
            )
