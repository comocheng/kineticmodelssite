...
class SourceImporter(Importer):
    ...
    def import_authorship(self):
        ...
        for order, dj_author, _ in authors:
            dj_authorship, authorship_created = Authorship.objects.get_or_create(
                author=dj_author, source=dj_source, order=order
            )
...
class ThermoLibraryImporter(Importer):
    ...
    def import_species(self):
        ...
        if not self.library:
            ...
            logger.warning(
                "Starting the import_species method on ThermoLibraryImporter with library {}".format(
                    library.name
                )
            )
        ...
        for entry in library.entries:
            ...
            try:  # TODO -- Write a helper function to encapsulate get_or_create in a try-except block
                ...
                if species_created:
                    logger.debug(
                        "Found no species for structure {} {}, so making one".format(
                            smiles, molecule.multiplicity
                        )
                    )
                    ...
                    logger.debug(
                        "Found a unique species {} for structure {} {}".format(
                            dj_species, smiles, molecule.multiplicity
                        )
                    )
                ...
                logger.warning(
                    f"ThermoLibraryImporter.import_species: Found {len(possible_species)} species for structure {smiles} "
    ...
    def import_data(self):
        ...
        if not self.library:
            ...
            library = self.library
            logger.warning(
                "Starting the import_data method on ThermoLibraryImporter with library {}".format(
                    library.name
                )
            )
        ...
        for entry in library.entries:
            ...
            dj_thermo, thermo_created = Thermo.objects.get_or_create(
                species=species, source=self.dj_km.source
            )  # Empty Thermo model instance from Django kineticmodelssite
            ...
            for nasaPolynomial in list(
                enumerate(thermoEntry.polynomials, start=1)
            ):  # <-- List of tuples (index, Poly)
                ...
                try:
                    setattr(
                        dj_thermo, "Tmin{}".format(nasaPolynomial[0]), nasaPolynomial[1].Tmin.value
                    )
                    logger.debug(
                        "Assigned value {1} to lowerTempBound{0}".format(
                            nasaPolynomial[0], nasaPolynomial[1].Tmin.value
                        )
                    )
                except Exception as e:
                    logger.error(
                        "Library {2} has an Error assigning the lower temperature bound to "
                ...
                try:
                    setattr(
                        dj_thermo, "Tmax{}".format(nasaPolynomial[0]), nasaPolynomial[1].Tmax.value
                    )
                    logger.debug(
                        "Assigned value {1} to upperTempBound{0}".format(
                            nasaPolynomial[0], nasaPolynomial[1].Tmax.value
                        )
                    )
                except Exception as e:
                    logger.error(
                        "Library {2} has an Error assigning the upper temperature bound to "
                        "Polynomial {1}:\n{0}\n".format(e, nasaPolynomial[0], library.name)
                    )
                    raise e
                ...
                for coefficient in list(
                    enumerate(nasaPolynomial[1].coeffs, start=1)
                ):  # <-- List of tuples (index, coeff)
                    ...
                    try:
                        ...
                        logger.debug(
                            "Assigned value {2} for coefficient {1}"
                            " to Polynomial {0}".format(
                                nasaPolynomial[0], coefficient[0], coefficient[1]
                            )
                        )
                    except Exception as e:
                        logger.error(
                            "Library {4} has an Error assigning the value {3} to coefficient {2} of "
                            "Polynomial {1}:\n{0}\n".format(
                                e, nasaPolynomial[0], coefficient[0], coefficient[1], library.name
                            )
                        )
                        raise e
...
class KineticsLibraryImporter(Importer):
    ...
    def load_library(self):
        ...
        try:
            library.load(
                fileName, local_context=local_context
            )  # Where dj_km attributes are renamed from entry() attr.
        except Exception as e:
            logger.error("Error reading {0}:".format(fileName), exc_info=True)
    ...
    def import_data(self):
        ...
        if not self.library:
            ...
            library = (
                self.library
            )  # refers to one of the "reactions.py" files in the RMG-models repository
            logger.warning(
                "Starting the import_data method on KineticsLibraryImporter with library {}".format(
                    library.name
                )
            )
        ...
        for entry in library.entries:  # refers to an "entry()" line in that file
            kinetics = library.entries[
                entry
            ].data  # "data" refers to the "kinetics" attribute of an "entry"
            chemkinReaction = library.entries[entry].item  # an rmg reaction
            print(chemkinReaction)
            # try:
            stoichiometries = {"forward": defaultdict(float), "reverse": defaultdict(float)}
            try:
            ...
            logger.info(
                f"We found the following reactions {reaction_list} in the database that `match` {chemkinReaction}"
            )
            ...
            try:
                if len(reaction_list) == 0:
                    ...
                    for reaction in reaction_list:
                        ...
                        if rmg_reaction.is_isomorphic(
                            chemkinReaction, either_direction=chemkinReaction.reversible
                        ):
                        ...
                        if found_match:
                            ...
                            if (
                                matched_reaction is not None
                            ):  # Checking that we matched only one reaction
                                raise Exception(
                                    f"Matched {chemkinReaction} to multiple reactions, {matched_reaction} and {reaction}"
                                )
                            matched_reaction = reaction
                    if not matched_reaction:
                        logger.info(
                            "Could not match reaction to existing reactions in our database, creating a new entry"
                        )
                        new_reaction = True
                        matched_reaction = Reaction.objects.create()
                if not matched_reaction:
                    logger.warning(
                        f"A matched reaction was not found or created for {chemkinReaction}, skipping for now..."
                    )
                    continue
            except Exception as e:
                print(e)
            ...
            if new_reaction:
                try:
                    ...
                    logger.error(
                        f"It appears that {chemkinReaction} isn't fully imported... skipping for now..."
                    )
            ...
            if isinstance(kinetics, KineticsData):
                dj_kinetics_data = KineticsData_dj.objects.get_or_create(
                    temp_array=kinetics.Tdata.__str__(), rate_coefficients=kinetics.Kdata.__str__()
                )  # make the django model instance

            elif isinstance(kinetics, Arrhenius):
                dj_kinetics_data = make_arrhenius_dj(
                    kinetics, library_name=library.name
                )  # use function to make model instance
                ...
                dj_kinetics_data = (
                    MultiArrhenius_dj.objects.create()
                )  # make the django model instance
                # No atomic data (numbers, strings, etc.,)
                save_model(
                    dj_kinetics_data, library_name=library.name
                )  # Have to save the model before you can ".add()" onto a ManyToMany
                for simple_arr in kinetics.arrhenius:
                    ...
                    dj_kinetics_data.arrhenius_set.add(
                        make_arrhenius_dj(simple_arr, library_name=library.name)
                    )

            elif isinstance(kinetics, MultiPDepArrhenius):
                dj_kinetics_data = (
                    MultiPDepArrhenius_dj.objects.create()
                )  # make the django model instance
                save_model(
                    dj_kinetics_data, library_name=library.name
                )  # Have to save the model before you can ".add()" onto a ManyToMany
                for pdep_arr in kinetics.arrhenius:
                    ...
                    dj_kinetics_data.pdep_arrhenius_set.add(
                        make_pdep_arrhenius_dj(pdep_arr, library_name=library.name)
                    )

            elif isinstance(kinetics, PDepArrhenius):
                make_pdep_arrhenius_dj(
                    kinetics, library_name=library.name
                )  # use function to make model instance
                ...
                dj_kinetics_data.low_arrhenius = make_arrhenius_dj(
                    kinetics.arrheniusLow, library_name=library.name
                )
                save_model(dj_kinetics_data, library_name=library.name)
                make_efficiencies(
                    dj_kinetics_data, efficiencies, self.dj_km, library_name=library.name
                )
                ...
                dj_kinetics_data.high_arrhenius = make_arrhenius_dj(
                    kinetics.arrheniusHigh, library_name=library.name
                )
                dj_kinetics_data.low_arrhenius = make_arrhenius_dj(
                    kinetics.arrheniusLow, library_name=library.name
                )
                ...
                logger.error(
                    "Library {} Cannot identify this type of Kinetics Data: "
            ...
            dj_kinetics_object, object_created = Kinetics.objects.get_or_create(
                reaction=matched_reaction, source=self.dj_km.source,
            )
...
def save_model(mod, library_name=None):
    try:
        ...
        logger.exception(
            f"Error saving the {mod} model type {type(mod)} from Library {library_name}"
        )
        raise
...
def make_arrhenius_dj(k, library_name=None):
    ...
    if k.Tmin is not None:
        min_temp = k.Tmin.value_si
    if k.Tmax is not None:
        max_temp = k.Tmax.value_si
    if k.Pmin is not None:
        min_pressure = k.Pmin.value_si
    if k.Pmin is not None:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Import RMG models into Django.")
    parser.add_argument(
        "paths",
        metavar="path",
        type=str,
